"""
CoinSifter — 技術指標計算模組
支援 RSI, MACD, EMA, BB, KD, ATR, Volume 等常用指標（純 Python 實現）
"""
import pandas as pd
import ccxt
import numpy as np
...
def calc_macd(df: pd.DataFrame, fast: int = 12, slow: int = 26,
    ...
    """計算 MACD，回傳 DataFrame 含 MACD, MACDh, MACDs"""
...
def calc_bbands(df: pd.DataFrame, period: int = 20,
    ...
    """計算布林帶，回傳 DataFrame 含 BBL, BBM, BBU"""
...
def compute_indicators(df: pd.DataFrame, indicators_config: list) -> dict:
    """
    根據配置計算所有需要的指標，回傳最新值的字典
    indicators_config: [{'name': 'rsi', 'period': 14}, ...]
    """
    results = {}
    # 用已收盤的 K 棒（iloc[-2]），避免未收盤價格跳動造成假信號
    ci = -2 if len(df) >= 2 else -1  # closed index
    last = df.iloc[ci]
    prev = df.iloc[ci - 1] if len(df) >= abs(ci) + 1 else last
    ...
    for cfg in indicators_config:
        ...
        if name == 'rsi':
            ...
            results['rsi'] = round(series.iloc[ci], 2) if pd.notna(series.iloc[ci]) else None

        elif name == 'ema':
            # 檢查 K 線數量是否足夠（至少需要 period 數量的 K 線 +緩衝）
            ...
            if len(df) < min_candles:
                ...
                val = series.iloc[ci]
                prev_val = series.iloc[ci - 1] if len(series) >= abs(ci) + 1 else None
            ...
            # 用已收盤的 K 棒判定（iloc[-2]），避免未收盤價格跳動造成假信號
            if len(ema20) >= 3 and len(ema50) >= 3:
                cur_20 = ema20.iloc[-2]   # 最近一根已收盤
                cur_50 = ema50.iloc[-2]
                prev_20 = ema20.iloc[-3]  # 前一根已收盤
            ...
            # 金叉：前一根 EMA20 <= EMA50，當前已收盤 EMA20 > EMA50
            # 新幣種 K 線不足 50 時不報 Golden Cross（EMA50 未穩定）
            min_candles = 50
            ...
            if bb_df is not None and not bb_df.empty:
                cols = bb_df.columns
                results['bb_lower'] = round(bb_df[cols[0]].iloc[ci], 8) if pd.notna(bb_df[cols[0]].iloc[ci]) else None
                results['bb_mid'] = round(bb_df[cols[1]].iloc[ci], 8) if pd.notna(bb_df[cols[1]].iloc[ci]) else None
                results['bb_upper'] = round(bb_df[cols[2]].iloc[ci], 8) if pd.notna(bb_df[cols[2]].iloc[ci]) else None

        elif name == 'kd':
            kd_df = calc_kd(df, period, cfg.get('d_period', 3))
            if kd_df is not None and not kd_df.empty:
                cols = kd_df.columns
                results['stoch_k'] = round(kd_df[cols[0]].iloc[ci], 2) if pd.notna(kd_df[cols[0]].iloc[ci]) else None
                results['stoch_d'] = round(kd_df[cols[1]].iloc[ci], 2) if pd.notna(kd_df[cols[1]].iloc[ci]) else None
                # 金叉/死叉：比較前一根已收盤 vs 當前已收盤
                prev_k = kd_df[cols[0]].iloc[ci - 1] if len(kd_df) >= abs(ci) + 1 else None
                prev_d = kd_df[cols[1]].iloc[ci - 1] if len(kd_df) >= abs(ci) + 1 else None
                cur_k = kd_df[cols[0]].iloc[ci]
                cur_d = kd_df[cols[1]].iloc[ci]
            ...
            results['atr'] = round(series.iloc[ci], 8) if pd.notna(series.iloc[ci]) else None

        elif name == 'volume':
            vol_period = cfg.get('period', 20)
            vol_ma = calc_vol_ma(df, vol_period)
            current_vol = last['v']
            avg_vol = vol_ma.iloc[ci] if pd.notna(vol_ma.iloc[ci]) else 0
